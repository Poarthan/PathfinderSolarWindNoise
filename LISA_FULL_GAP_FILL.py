...
if truncate==True:
    file = open(f'datafiles/LISA_full_gf.txt',"a")
    file.truncate(0)
    file.close()
...
def main():
    ...
    while finished==False:
        ...
        if not bdlen:
            n1=bad[3]-gap_count//2
            n2=bad[3]+gap_count//2
        else:
            fill_count=int(ifbad/diffthreshold)
            n1=bad[3]-fill_count//2
            n2=bad[3]+fill_count//2
    ...
    print("writing to", 'datafiles/LISA_full_gf.txt')
...
def write_file(time, gap_fill):
    file = open('datafiles/LISA_full_gf.txt',"a")
    file.write(str(time))
    file.write("\t")
