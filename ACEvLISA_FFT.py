...
#    plot_(ace, big, at, lt)
    ...
    print("a")
    plt.semilogy(np.abs(freqs[:N//2]), np.abs(sig_fft[:N//2]))
    print("a")
    ...
    markerline, stemlines, baseline = plt.stem(freqs[:N], np.abs(sig_fft[:N]), '-')
    N=sig_fft1.size
    markerline, stemlines, baseline = plt.stem(freq1[:N], np.abs(sig_fft1[:N]), '-')
    plt.setp(stemlines, 'linewidth', 1)
    print("a")
    plt.semilogy(np.abs(freq1), np.abs(sig_fft1))
    print("a")
    ###labels code is made, but kind of ugly, so just title for now
    ftypes=['jpg']
    #ftypes=['png']
    saveplot(f'plots/FFT_Comparison_{filea[10:-4]}_and_{fileb[10:-4]}', ftypes)
    plt.show()
